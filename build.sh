...
if [ -f "www/versions.json" ]; then
	echo "   Preserving existing www/versions.json"
elif [ -f "$VERSIONS_SRC" ]; then
...
for v in $VERSIONS; do
	...
	mkdir -p "www/$v/locale"
	rsync -a --delete "www/locale/" "www/$v/locale/"
done
...
# (config.xml also declares it as <resource-file>, so cordova prepare copies it as well)
mkdir -p platforms/android/app/src/main/res/raw/
cp cert/server_cert.der platforms/android/app/src/main/res/raw/server_cert.der
...
# The plain `cordova build android --release` only produces the AAB (bundleRelease);
# the APK task stays up-to-date/stale. Build the APK explicitly afterwards so
# `adb install` below picks up an APK containing the freshly synced version folders.
...
# Clean up dynamically packaged UI versions from git-tracked workspace while preserving any
# local versions.json edits made for the current build.
rm -rf www/[0-9]*
