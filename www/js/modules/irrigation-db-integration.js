...
OSApp.Analog.IrrigationDB = {
    ...
    getRecommendations: function(zone, plant, callback) {
        $.ajax({
            ...
            error: function(xhr) {
                console.error('Error loading recommendations:', xhr);
                callback(null);
            }
        });
    },
    ...
    /**
     * Get irrigation settings (min/max) for specific plant and zone
     */
    getIrrigationSettings: function(zone, plant, callback) {
        $.ajax({
            url: this.apiUrl,
            data: {
    ...
    showDialog: function(onSelect) {
        ...
        this.getZones(function(zones) {
            ...
            popup.find("#irrigdb-search").on("click", function() {
                ...
                self.getIrrigationSettings(zone, plant, function(data) {
                    var resultsDiv = popup.find("#irrigdb-results");
                    resultsDiv.empty();

                    if (!data) {
                        resultsDiv.html("<p style='color:#999;'>No results found. Make sure database has data for this plant and zone.</p>").show();
                        return;
                    }

                    // If single result object, convert to array
                    ...
                    var html = "<h3>Results (" + results.length + ")</h3>" +
                    ...
                    results.forEach(function(item) {
                        // Handle both min_value/max_value and start_irrigation/stop_irrigation
                        var minValue = item.min_value !== undefined ? item.min_value : item.start_irrigation;
                        var maxValue = item.max_value !== undefined ? item.max_value : item.stop_irrigation;
                        var category = item.category_name || item.plant_category || 'Unknown';

                        html += "<tr style='border-bottom:1px solid #eee;'>" +
                            "<td style='padding:8px;'><strong>" + item.plant_name + "</strong><br>" +
                            "<small>" + category + "</small></td>" +
                            "<td style='padding:8px; text-align:center;'><strong>" + minValue + "</strong></td>" +
                            "<td style='padding:8px; text-align:center;'><strong>" + maxValue + "</strong></td>" +
                            "<td style='padding:8px; text-align:center;'>" +
                            "<button class='irrigdb-select' data-min='" + minValue + "' " +
                            "data-max='" + maxValue + "' data-plant='" + item.plant_name + "' " +
...
$(document).ready(function() {
    if (typeof OSApp.Analog !== 'undefined' && typeof OSApp.Analog.IrrigationDB !== 'undefined') {
