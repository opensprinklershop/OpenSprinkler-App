...
OSApp.Options.showOptions = function( expandItem ) {
	var list = "",
		...
		shouldSkipOptionControl = function( $item ) {
			...
			// The #wto button is a hidden data carrier for the weather provider and
			// weather restriction settings, which are edited through always-visible
			// controls. Its field-contain is hidden when the adjustment method is
			// Manual, so never skip it or those changes would be silently dropped.
			if ( $item.attr( "id" ) === "wto" ) {
				return false;
			}
	...
	if ( nativePushAvail ) {
		...
		if ( OSApp.Push && typeof OSApp.Push.getBaseUrl === "function" ) {
			...
			list += "<div class='ui-field-contain'><label for='push-endpoint'>" + OSApp.Language._( "Push Service URL" ) +
					"<button data-helptext='" +
						OSApp.Language._( "Optional. URL of the OpenSprinkler push forwarder used to deliver notifications when the app is closed. Leave empty to disable server push." ) +
						"' class='help-icon btn-no-border ui-btn ui-icon-info ui-btn-icon-notext'></button>" +
				"</label>" +
